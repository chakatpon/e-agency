...
function ForgotPassword() {
    ...
    const handleForgotPassword = async (e) => {
        ...
        else if(Utils.ValidateEmail(strEmail) != "")
        {
            ...
            const params = { method: 'GET', url: CONFIG.AGENCY_APIS.URL + '/Agents/ForgotPassword?LoginName=' + strLoginName + '&Email=' + strEmail }
            //console.log(params);
            try {
                    ...
                    <form className="pd-t-2" >
                        ...
                        <label htmlFor="notation" className="col-sm-4 col-form-label text-left text-sm-right" >{"หมายเหตุ :"}</label>
                            <div className="col-sm-8">
                                <div>
                                <p className="text-left" id="notation">{"กรุณาใส่รหัสผู้ใช้ (loginname) และอีเมลแอดเดรสของท่าน แล้วคลิกที่ปุ่ม ส่งรหัสผ่าน ระบบ e-Agency จะ Generate รหัสผ่านให้ท่านใหม่แล้วส่งไปยังอีเมลแอดเดรสที่ได้ลงทะเบียนไว้"}</p>
                                </div>
                                <div>
                                <p className="text-left" id="notation">{"* หากไม่ทราบรหัสผู้ใช้หรืออีเมลแอดเดรส กรุณาติดต่อเจ้าหน้าที่ดูแลระบบ e-Agency" }</p>
                                </div>
                            </div>
